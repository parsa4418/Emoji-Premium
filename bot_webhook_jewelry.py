...
if __name__ == "__main__":
    # Render URL is supplied automatically; never hard-code the old webhook URL.
    render_url = os.environ.get("RENDER_EXTERNAL_URL", "").strip().rstrip("/")
    if not render_url:
        raise RuntimeError("RENDER_EXTERNAL_URL is not set on Render")

    WEBHOOK_URL = f"{render_url}/webhook"

    # Replace any webhook belonging to an older deployment, then register only
    # the current Render service URL. No previous/hard-coded URL is used.
    bot.remove_webhook()
    bot.set_webhook(
        url=WEBHOOK_URL,

...
def _gift_value_from_id(gift_id, gift_code):
    gid = max(1, min(int(gift_id), GIFT_MAX_PER_TYPE))
    # ارزش هر نسخه به‌صورت قطعی بر اساس آیدی همان مدل محاسبه می‌شود.
    rarity_factor = (GIFT_MAX_PER_TYPE - gid) / max(1, GIFT_MAX_PER_TYPE - 1)
    code_factor = (int(str(gift_code)[-6:]) % 1_000_000) / 1_000_000
    value = (
        GIFT_MIN_VALUE
        + int(rarity_factor * 140_000_000)
        + int(code_factor * 20_000_000)
    )
    return max(GIFT_MIN_VALUE, min(GIFT_MAX_VALUE, value))

def _premium_gift_ready():
    try:
        supabase.table("premium_gifts").select("instance_id").limit(1).execute()
        return True
    except Exception as exc:
        logging.error("premium_gifts table unavailable: %s", exc)
        return False

def _seed_or_normalize_premium_gifts():
    """
    برای هر مدل (gift_code) دقیقاً تا 10 نسخه نگه می‌دارد:
    آیدی‌های هر مدل 1..10 هستند و بین مدل‌های مختلف تکرارشان کاملاً مجاز است.
    اگر قبلاً فقط یک نسخه از هر مدل seed شده باشد، نسخه‌های 2..10 خودکار ساخته می‌شوند.
    """
    try:
        rows = (
            supabase.table("premium_gifts")
            .select("instance_id,gift_code,gift_id,owner_id")
            .order("instance_id")
            .execute()
            .data
            or []
        ...
        by_code = {}
        for row in rows:
            by_code.setdefault(int(row.get("gift_code") or 0), []).append(row)

        payload = []
        all_codes = list(dict.fromkeys(
            [int(code) for code in PREMIUM_GIFT_CODES] + list(by_code.keys())
        ))

        for gift_code in all_codes:
            existing = by_code.get(gift_code, [])
            used_ids = set()

            # نسخه‌های فعلی را به 1..10 نگاشت می‌کنیم.
            for index, row in enumerate(existing[:GIFT_MAX_PER_TYPE], start=1):
                used_ids.add(index)
                if int(row.get("gift_id") or 0) != index:
                    supabase.table("premium_gifts").update(
                        {"gift_id": int(index)}
                    ).eq("instance_id", row["instance_id"]).execute()

            # هر نسخه‌ی گمشده‌ی همان مدل را می‌سازیم.
            for gift_id in range(1, GIFT_MAX_PER_TYPE + 1):
                if gift_id in used_ids:
                    continue
                value = _gift_value_from_id(gift_id, gift_code)
                payload.append({
                    "gift_code": int(gift_code),
                    "gift_id": int(gift_id),
                    "owner_id": None,
                    "current_value": int(value),
                    "base_value": int(value),
                    "acquired_price": int(value),
                    "active": False,
                })
        ...
        if payload:
            ...
            for i in range(0, len(payload), chunk_size):
                supabase.table("premium_gifts").insert(
                    payload[i:i + chunk_size]
                ).execute()

        logging.info(
            "Premium gift inventory ready: %s gift models, up to %s copies per model.",
            len(all_codes),
            GIFT_MAX_PER_TYPE,
        )
    except Exception as exc:
        logging.exception("Premium gift inventory setup failed: %s", exc)

def _gift_free_rows(limit=1000):
    try:
        return (
            supabase.table("premium_gifts")
            .select("*")
        ...
        logging.error("premium gift free rows: %s", exc)
        return []

def gift_box_text(uid):
    free_rows = _gift_free_rows()
    stock_types = {}
    for row in free_rows:
        code = int(row.get("gift_code") or 0)
        stock_types[code] = stock_types.get(code, 0) + 1
    stock = sum(stock_types.values())
    return (
        "🎁 گیفت‌های پرمیوم\n\n"
        f"💎 قیمت هر گیفت: {GIFT_BOX_PRICE:,} الماس\n"
        f"📦 موجودی کل: {stock:,} عدد\n\n"
        "هر مدل گیفت ۱۰ نسخه دارد و آیدی همان مدل از ۱ تا ۱۰ است.\n"
        "برای مثال، گیفت سگ نوع A می‌تواند آیدی ۶ داشته باشد و گیفت سگ نوع B هم آیدی ۶ داشته باشد.\n\n"
        f"💰 موجودی شما: {get_balance(uid):,} 💎"
    )

def _gift_buy(uid):
    if not _premium_gift_ready():
        return False, "❌ جدول premium_gifts ساخته نشده است."
    if get_balance(uid) < GIFT_BOX_PRICE:
        return False, f"❌ موجودی کافی نیست. قیمت گیفت {GIFT_BOX_PRICE:,} 💎 است."

    free_rows = _gift_free_rows()
    if not free_rows:
        return False, "❌ فعلاً هیچ گیفتی باقی نمانده است."

    # یک نسخه آزاد را تصادفی انتخاب می‌کنیم، ولی gift_id همان نسخه 1..10 باقی می‌ماند.
    row = random.choice(free_rows)
    gift_code = int(row["gift_code"])
    gift_id = int(row["gift_id"])
    value = _gift_value_from_id(gift_id, gift_code)

    if update_diamonds(uid, -GIFT_BOX_PRICE) is None:
        return False, "❌ کسر مبلغ انجام نشد."
    ...
    try:
        updated = (
            ...
            .update({
                ...
                "acquired_price": int(value),
                "active": False,
                "acquired_at": datetime.now(TEHRAN_TZ).isoformat(),
            })
        ...
        if not updated.data:
            ...
            return False, "❌ این گیفت همزمان توسط کاربر دیگری گرفته شد."
        return True, updated.data[0]
    except Exception as exc:
        update_diamonds(uid, GIFT_BOX_PRICE)
        logging.error("gift buy: %s", exc)
        return False, "❌ خرید گیفت ناموفق بود."
...
def display_gift_for_user(uid):
    gift = get_active_gift(uid)
    if not gift:
        return ""
    return _gift_emoji(gift["gift_code"], "✨")

def gifts_text(uid):
    gifts = get_user_gifts(uid)
    if not gifts:
        return "🎁 هنوز هیچ گیفتی نداری."

    lines = ["🎁 گیفت‌های شما", ""]
    for gift in gifts:
        code = int(gift.get("gift_code") or 0)
        gid = int(gift.get("gift_id") or 0)
        value = int(gift.get("current_value") or 0)
        active = " ✅ فعال" if gift.get("active") else ""
        lines.append(
            f"{_gift_emoji(code, '🎁')} | آیدی: {gid} | ارزش: {value:,} 💎{active}"
        )
    return "\n".join(lines)

def set_active_gift(uid, instance_id):
    try:
        own = (
            supabase.table("premium_gifts")
            .select("*")
            .eq("instance_id", int(instance_id))
            .eq("owner_id", int(uid))
            .limit(1)
            .execute()
        )
        if not own.data:
            return False, "❌ این گیفت متعلق به شما نیست."
        ...
        return True, own.data[0]
    except Exception as exc:
        logging.error("set_active_gift: %s", exc)
        return False, "❌ خطا در فعال‌سازی گیفت."

def sell_gift(uid, instance_id):
    try:
        result = (
            supabase.table("premium_gifts")
            .select("*")
        ...
        price = int(result.data[0].get("current_value") or 0)
        (
            supabase.table("premium_gifts")
            .update({
                "owner_id": None,
        ...
        update_diamonds(uid, price)
        return True, price
    except Exception as exc:
        logging.error("sell_gift: %s", exc)
        return False, "❌ فروش گیفت ناموفق بود."

def transfer_gift(sender_id, target_id, instance_id):
    try:
        if int(sender_id) == int(target_id):
            return False, "❌ نمی‌توانی گیفت را به خودت منتقل کنی."

        if not get_user(target_id):
            return False, "❌ کاربر مقصد هنوز /start نزده است."

        result = (
            supabase.table("premium_gifts")
            .select("*")
            .eq("instance_id", int(instance_id))
            .eq("owner_id", int(sender_id))
            .limit(1)
            .execute()
        )
        if not result.data:
        ...
        logging.error("transfer_gift: %s", exc)
        return False, "❌ انتقال گیفت ناموفق بود."

def update_gift_market():
    try:
        rows = (
            supabase.table("premium_gifts")
            .select("instance_id,current_value")
        ...
        for row in rows:
            current = int(row.get("current_value") or GIFT_MIN_VALUE)
            new_value = max(
                GIFT_MIN_VALUE,
                min(
                    GIFT_MAX_VALUE,
                    current + random.randint(-GIFT_MAX_PRICE_DOWN, GIFT_MAX_PRICE_UP),
                ),
            )
            supabase.table("premium_gifts").update({
                "current_value": int(new_value),
                "last_market_update": datetime.now(TEHRAN_TZ).isoformat(),
            }).eq("instance_id", row["instance_id"]).execute()
    except Exception as exc:
        logging.error("gift market: %s", exc)

def _premium_gift_main_markup(uid):
    markup = types.InlineKeyboardMarkup()
    markup.row(
        colored_button("🎁 خرید گیفت", callback_data=f"giftbox|{uid}"),
        colored_button("🎒 گیفت‌های من", callback_data=f"giftmine|{uid}"),
    )
    markup.row(colored_button("🏠 منوی اصلی", callback_data=f"mainmenu|{uid}"))
    return markup

@bot.callback_query_handler(
    func=lambda call: call.data == "giftbox" or call.data.startswith("giftbox|")
)
def premium_gift_box_callback(call):
    uid = check_panel_owner(call) if "|" in call.data else call.from_user.id
    if uid is None:
        return
    if not get_user(uid):
    ...
    safe_edit_message(
        ...
        call.message.chat.id,
        call.message.message_id,
        reply_markup=_premium_gift_main_markup(uid),
        parse_mode="HTML",
    )

@bot.message_handler(func=lambda m: text_is(m, "گیفت", "گیفت پرمیوم", "گیفت‌ها", "گیفت باکس"))
def text_premium_gift(message):
    ...
    show_or_edit_panel(
        ...
        "premium_gifts",
        gift_box_text(uid),
        _premium_gift_main_markup(uid),
        parse_mode="HTML",
    )

@bot.callback_query_handler(func=lambda call: call.data.startswith("giftbuy|"))
def premium_gift_buy_callback(call):
    uid = check_panel_owner(call)
    if uid is None:
        return
    ok, result = _gift_buy(uid)
    if not ok:
        bot.answer_callback_query(call.id, result, show_alert=True)
        return

    gift_code = int(result["gift_code"])
    gift_id = int(result["gift_id"])
    value = int(result.get("current_value") or 0)

    bot.answer_callback_query(call.id, "✅ گیفت با موفقیت خریداری شد.")
    safe_edit_message(
        f"{_gift_emoji(gift_code, '🎁')} گیفت جدید شما\n\n"
        f"🆔 آیدی گیفت: {gift_id}\n"
        f"💎 ارزش گیفت: {value:,}\n\n"
        "این آیدی در هر مدل جداگانه است و می‌تواند در مدل دیگری دوباره تکرار شود.",
        call.message.chat.id,
        call.message.message_id,
        reply_markup=_premium_gift_main_markup(uid),
        parse_mode="HTML",
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("giftmine|"))
def premium_gift_mine_callback(call):
    uid = check_panel_owner(call)
    if uid is None:
        return
    ...
    for gift in gifts[:50]:
        ...
        markup.row(
            colored_button(
                f"{_gift_emoji(code, '🎁')} آیدی {gid} | {value:,} 💎",
                callback_data=f"giftshow|{uid}|{gift['instance_id']}",
            )
        )
    markup.row(colored_button("🎁 بخش گیفت", callback_data=f"giftbox|{uid}"))
    markup.row(colored_button("🏠 منوی اصلی", callback_data=f"mainmenu|{uid}"))
    bot.answer_callback_query(call.id)
    safe_edit_message(
        gifts_text(uid),
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("giftshow|"))
def premium_gift_show_callback(call):
    uid = check_panel_owner(call)
    if uid is None:
        return
    try:
        ...
        bot.answer_callback_query(call.id, "❌ گیفت نامعتبر.", show_alert=True)
        return

    gifts = [
        gift for gift in get_user_gifts(uid)
        if int(gift.get("instance_id")) == instance_id
    ]
    if not gifts:
        bot.answer_callback_query(call.id, "❌ گیفت پیدا نشد.", show_alert=True)
        return
    ...
    gid = int(gift.get("gift_id") or 0)
    value = int(gift.get("current_value") or 0)
    active = bool(gift.get("active"))

    markup = types.InlineKeyboardMarkup()
    markup.row(
        colored_button(
            "👤 نمایش کنار اسم",
            callback_data=f"giftactive|{uid}|{instance_id}",
        )
    )
    markup.row(
        colored_button(
            "💰 فروش",
            callback_data=f"giftsell|{uid}|{instance_id}",
        ),
        colored_button(
            "🔄 انتقال",
            callback_data=f"gifttransfer|{uid}|{instance_id}",
        ),
    )
    markup.row(colored_button("🔙 گیفت‌های من", callback_data=f"giftmine|{uid}"))

    safe_edit_message(
        f"{_gift_emoji(code, '🎁')} گیفت شما\n\n"
        f"🆔 آیدی گیفت: {gid}\n"
        f"💎 ارزش فعلی: {value:,}\n"
        f"📌 وضعیت: {'فعال کنار اسم' if active else 'غیرفعال'}",
        call.message.chat.id,
        call.message.message_id,
        reply_markup=markup,
        parse_mode="HTML",
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("giftactive|"))
def premium_gift_activate_callback(call):
    uid = check_panel_owner(call)
    if uid is None:
        return
    instance_id = int(call.data.split("|")[2])
    ok, result = set_active_gift(uid, instance_id)
    bot.answer_callback_query(
        call.id,
        "✅ گیفت کنار اسم فعال شد." if ok else result,
        show_alert=not ok,
    )
    if ok:
        premium_gift_show_callback(call)

@bot.callback_query_handler(func=lambda call: call.data.startswith("giftsell|"))
def premium_gift_sell_callback(call):
    uid = check_panel_owner(call)
    if uid is None:
        return
    instance_id = int(call.data.split("|")[2])
    ok, result = sell_gift(uid, instance_id)
    if not ok:
        bot.answer_callback_query(call.id, result, show_alert=True)
        return
    bot.answer_callback_query(call.id, f"✅ {int(result):,} 💎 دریافت شد.")
    safe_edit_message(
        gifts_text(uid),
        call.message.chat.id,
        call.message.message_id,
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("gifttransfer|"))
def premium_gift_transfer_callback(call):
    uid = check_panel_owner(call)
    if uid is None:
        return
    instance_id = int(call.data.split("|")[2])
    safe_edit_message(
        "🔄 آیدی عددی گیرنده را در پاسخ همین پیام بفرست.",
        call.message.chat.id,
        call.message.message_id,
        reply_markup=None,
    )
    register_timed_next_step_handler(
        call.message,
        premium_gift_transfer_step,
        uid,
        instance_id,
        call.message.message_id,
...
def premium_gift_transfer_step(message, expected_user_id, instance_id, panel_msg_id):
    if message.from_user.id != expected_user_id:
        return
    try:
        target = int(_normalize_digits(message.text.strip()))
    except Exception:
        safe_edit_message(
    ...
    ok, result = transfer_gift(expected_user_id, target, instance_id)
    safe_edit_message(
        "✅ گیفت با موفقیت منتقل شد." if ok else f"❌ {result}",
        message.chat.id,
        panel_msg_id,
        reply_markup=_premium_gift_main_markup(expected_user_id),
    )
    if ok:
        try:
            safe_send_message(target, "🎁 یک گیفت برایت منتقل شد! پیوی خودت را چک کن.")
        except Exception:
            pass
...
def display_name_with_tag(user_id, display_name):
    ...
    try:
        gift_badge = display_gift_for_user(user_id)
        if gift_badge:
            return f"{gift_badge} {name}"
    except Exception:
        pass
    return name
...
def main_menu_markup(user_id=None):
    ...
    markup.row(colored_button("🎁 گیفت‌ها", callback_data=f"giftbox|{user_id}" if user_id else "giftbox"))
...
# ------------------ Premium gifts startup initialization ------------------
_seed_or_normalize_premium_gifts()
try:
    premium_gift_scheduler = BackgroundScheduler(timezone=TEHRAN_TZ)
    premium_gift_scheduler.add_job(
        update_gift_market,
        "interval",
        seconds=GIFT_MARKET_INTERVAL_SECONDS,
        id="premium_gift_market",
        replace_existing=True,
        max_instances=1,
        coalesce=True,
    )
    premium_gift_scheduler.start()
except Exception:
    premium_gift_scheduler = None
...
if __name__ == "__main__":
    # Render hostname is used so no previous service URL remains in the source.
    render_host = os.environ.get("RENDER_EXTERNAL_HOSTNAME", "emoji-blp5.onrender.com").strip()
    WEBHOOK_URL = f"https://{render_host}/webhook"
    try:
        # Do not clear the webhook first: clearing it can make a valid deployment
        # stop receiving updates and is unnecessary when setting the new URL.
        bot.set_webhook(
            url=WEBHOOK_URL,
        ...
        logging.exception("Failed to configure Telegram webhook: %s", exc)
        raise

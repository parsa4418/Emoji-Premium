...
def register_channel(user_id, channel_username, price):
    """Atomically check balance, deduct price, and register channel."""
    with db() as conn:
        with conn.cursor() as cur:
...
def set_user_channel(user_id, channel_username):
    """Store exactly one destination channel for a user."""
    with db() as conn:
        with conn.cursor() as cur:

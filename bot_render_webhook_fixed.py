...
def gift_mine_callback(call):
    ...
    try:
        ...
        for g in gs[:50]:
            m.row(colored_button(f"{int(g['gift_id']):,} | {int(g['current_value']):,} 💎",callback_data=f"giftshow|{uid}|{g['instance_id']}",icon_custom_emoji_id=g['gift_code']))

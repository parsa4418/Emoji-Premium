...
def gift_mine_callback(call):
    ...
    for g in gs[:50]:
        tier_emoji = gift_rarity_name(g['current_value']).split(" ")[0]
        m.row(colored_button(f"{tier_emoji} {int(g['gift_id']):,} | {int(g['current_value']):,} 💎",callback_data=f"giftshow|{uid}|{g['instance_id']}"))

...
def _gift_value_from_id(gift_id,gift_code):
    gid=max(1,min(int(gift_id),999_999_999)); rarity=(999_999_999-gid)/999_999_999; factor=(int(str(gift_code)[-6:]) % 1_000_000)/1_000_000
    return max(GIFT_MIN_VALUE,min(GIFT_MAX_VALUE,GIFT_MIN_VALUE+int(rarity*170_000_000)+int(factor*20_000_000)))
...
def _gift_free():
    try:
        rows=supabase.table("premium_gifts").select("*").is_("owner_id","null").gte("gift_id",1).lte("gift_id",GIFT_MAX_PER_TYPE).limit(1000).execute().data or [];return random.choice(rows) if rows else None
    except Exception as e:logging.error(f"خطا در _gift_free: {e}");return None

def gift_box_text(uid):
    return f"{_gift_emoji(5927029738626359516)} گیفت شانسی\n\n{_gift_emoji(5829923384217050622)} با خرید گیفت باکس برای خود یا دوستان میویی، شانست را برای به دست آوردن یک عدد گیفت میویی امتحان کن\n{_gift_emoji(5823445637231814311)} قیمت گیفت باکس شانسی : {GIFT_BOX_PRICE:,}\n{_gift_emoji(5825623013852192219)} الماس هات : {get_balance(uid):,}\n\n{_gift_emoji(5818716826699307883)} جهت خرید گیفت باکس از گزینه‌های زیر استفاده کن"

def count_owned_by_code(uid,gift_code):
    """تعداد گیفت‌هایی که کاربر از یک نوع مشخص (gift_code) در اختیار داره."""
    try:
...
def _gift_buy(uid):
    ...
    # توجه: gift_id (شماره‌ی نسخه، بین ۱ تا GIFT_MAX_PER_TYPE) از قبل روی همین ردیف
    # توسط اسکریپت seed دیتابیس مشخص شده؛ اینجا فقط همون رو می‌خونیم، نه اینکه یک
    # عدد تصادفی جدید بسازیم. این باعث می‌شه شماره‌ی گیفت هیچ‌وقت از GIFT_MAX_PER_TYPE بیشتر نشه.
    if row.get("gift_id") is None or row.get("gift_code") is None:
        logging.error(f"ردیف نامعتبر premium_gifts (gift_id/gift_code خالی): instance_id={row.get('instance_id')}")
        return False,"❌ این نسخه گیفت معتبر نیست، لطفاً دوباره امتحان کن."
    gid=int(row["gift_id"]);code=int(row["gift_code"]);value=_gift_value_from_id(gid,code)
    try:
        ...
        if not up.data:update_diamonds(uid,GIFT_BOX_PRICE);return False,"❌ گیفت همزمان توسط فرد دیگری گرفته شد."
        return True,up.data[0]
    except Exception as e:update_diamonds(uid,GIFT_BOX_PRICE);logging.error("gift buy: %s",e);return False,"❌ باز کردن گیفت ناموفق بود."
...
def get_active_gift(uid):
    ...
    except Exception as e:logging.error(f"خطا در get_active_gift({uid}): {e}");return None
...
def sell_gift(uid,iid):
    try:
        r=supabase.table("premium_gifts").select("*").eq("instance_id",int(iid)).eq("owner_id",int(uid)).limit(1).execute()
        if not r.data:return False,"این گیفت متعلق به شما نیست."
        price=int(r.data[0].get("current_value",0));supabase.table("premium_gifts").update({"owner_id":None,"active":False}).eq("instance_id",int(iid)).execute();update_diamonds(uid,price);return True,price
...
def update_gift_market():
    try:
        rows=supabase.table("premium_gifts").select("instance_id,current_value").not_.is_("owner_id","null").execute().data or []
        for r in rows:
            cur=int(r.get("current_value",GIFT_MIN_VALUE) or GIFT_MIN_VALUE);new=max(GIFT_MIN_VALUE,min(GIFT_MAX_VALUE,cur+random.randint(-GIFT_MAX_PRICE_DOWN,GIFT_MAX_PRICE_UP)));supabase.table("premium_gifts").update({"current_value":new,"last_market_update":datetime.now(TEHRAN_TZ).isoformat()}).eq("instance_id",r["instance_id"]).execute()
...
def gift_mine_callback(call):
    ...
    try:
        ...
        for g in gs[:50]:
            m.row(colored_button(f"{_safe_gift_id(g):,} | {int(g['current_value']):,} 💎",callback_data=f"giftshow|{uid}|{g['instance_id']}",icon_custom_emoji_id=g['gift_code']))
...
def gift_show_callback(call):
    ...
    if uid is None:return
    try:
        iid=int(call.data.split("|")[2]);gs=[g for g in get_user_gifts(uid) if int(g["instance_id"])==iid]
        if not gs:bot.answer_callback_query(call.id,"گیفت پیدا نشد.",show_alert=True);return
        g=gs[0];m=types.InlineKeyboardMarkup();m.row(colored_button("👤 نمایش کنار اسم",callback_data=f"giftactive|{uid}|{iid}"));m.row(colored_button("💰 فروش گیفت",callback_data=f"giftsell|{uid}|{iid}"),colored_button("🔄 انتقال گیفت",callback_data=f"gifttransfer|{uid}|{iid}"));m.row(colored_button("🔙 گیفت‌های من",callback_data=f"giftmine|{uid}"))
...
def gift_active_callback(call):
    uid=check_panel_owner(call)
    if uid is None:return
    ok,r=set_active_gift(uid,int(call.data.split("|")[2]));bot.answer_callback_query(call.id,"گیفت کنار اسمت فعال شد ✨" if ok else r,show_alert=not ok)

@bot.callback_query_handler(func=lambda c:c.data.startswith("giftsell|"))
def gift_sell_callback(call):
    uid=check_panel_owner(call)
    if uid is None:return
    ok,r=sell_gift(uid,int(call.data.split("|")[2]));bot.answer_callback_query(call.id,f"گیفت به {r:,} 💎 فروخته شد." if ok else r,show_alert=not ok);safe_edit_message(gifts_text(uid),call.message.chat.id,call.message.message_id,reply_markup=None,parse_mode="HTML")

@bot.callback_query_handler(func=lambda c:c.data.startswith("gifttransfer|"))
def gift_transfer_prompt(call):
    uid=check_panel_owner(call)
    if uid is None:return
    iid=int(call.data.split("|")[2]);safe_edit_message("🔄 آیدی عددی گیرنده را در پاسخ همین پیام بفرست.",call.message.chat.id,call.message.message_id);register_timed_next_step_handler(call.message,gift_transfer_step,uid,iid,call.message.message_id,expected_user_id=uid)

...
async def send_real_custom_emoji_message(user_id, text):
    """Send [custom_emoji_id] placeholders as real Telegram custom-emoji entities.

    Important Telegram rules:
    - The entity must wrap exactly the emoji contained in documentAttributeCustomEmoji.alt.
    - Entity offsets/lengths are UTF-16 code-unit based.
    - Premium-only custom emojis cannot be sent by a non-Premium user account.

    The function deliberately never falls back to plain 😎 for conversion messages,
    because doing so would destroy the original custom-emoji entity needed for forwarding.
    """
    if telethon_client is None:
        raise RuntimeError("MTProto not ready")
    if not text:
    ...
    if not matches:
        # No custom emojis: send normal text unchanged.
        return await telethon_client.send_message(user_id, text)

    # Resolve all custom emoji documents first. This avoids partially-built text/entities.
    document_by_id = {}
    ids = list(dict.fromkeys(int(m.group(1)) for m in matches))
    docs = await telethon_client(GetCustomEmojiDocumentsRequest(ids))
    for doc in docs or []:
        document_by_id[int(doc.id)] = doc
    ...
    for match in matches:
        ...
        doc = document_by_id.get(eid)
        if doc is None:
            raise ValueError(f"Custom emoji document not found: {eid}")
        ...
        for attr in getattr(doc, "attributes", []) or []:
            if hasattr(attr, "alt") and attr.alt:
                alt = attr.alt
            if hasattr(attr, "free"):
                is_free = bool(attr.free)

        if not alt:
            raise ValueError(f"Custom emoji alt not found: {eid}")

        # Telegram measures entity offsets in UTF-16 code units.
        current = "".join(parts)
        offset = len(current.encode("utf-16-le")) // 2
        length = len(alt.encode("utf-16-le")) // 2
        if length <= 0:
            raise ValueError(f"Custom emoji alt has zero length: {eid}")
        ...
        logging.error(
            "Custom emoji prepared id=%s free=%s alt=%r offset=%s length=%s",
            eid, is_free, alt, offset, length
        )

    parts.append(text[last:])
    clean = "".join(parts)

    # Final bounds validation against the exact UTF-16 text Telegram receives.
    total_utf16 = len(clean.encode("utf-16-le")) // 2
    for ent in entities:
        if ent.offset < 0 or ent.length <= 0 or ent.offset + ent.length > total_utf16:
            raise ValueError(
                f"Invalid custom emoji entity bounds: offset={ent.offset}, "
                f"length={ent.length}, total_utf16={total_utf16}"
            )
    ...
    # Read the message back and verify Telegram actually stored the entity.
    check = await telethon_client.get_messages(user_id, ids=sent.id)
    stored = getattr(check, "entities", None) or []
    stored_custom = [e for e in stored if isinstance(e, MessageEntityCustomEmoji)]
    if len(stored_custom) != len(entities):
        raise RuntimeError(
            f"Telegram did not preserve custom emoji entities: "
            f"sent={len(entities)} stored={len(stored_custom)}"
        )

    logging.error("CUSTOM ENTITY VERIFIED: message_id=%s count=%s", sent.id, len(stored_custom))
    return sent
...
async def show_convert_emoji(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not has_active_subscription(user.id):
        ...
        if not channels:
            ...
            text = ('برای ارسال پست، متن خودت را همراه با آیدی ایموجی‌ها بفرست.\n'
...
async def buy_account(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if has_active_subscription(user.id):
        ...
        if channels:
            ...
            return
        # Compatibility for an old active subscription that has no channel yet.
        context.user_data['waiting_for_channel_purchase'] = True
        text = ('اشتراک شما هنوز فعال است، اما کانالی برای آن ثبت نشده.\n\n'
                'آیدی کانال را با @ بفرست و ربات را در کانال <b>ادمین کامل</b> کن.\n'
                'برای این ثبتِ اولیه هزینه‌ای دوباره کم نمی‌شود.')
        await query.edit_message_text(text, parse_mode="HTML", reply_markup=create_back_keyboard())
        return
...
async def send_post_via_private_forward(update: Update, context: ContextTypes.DEFAULT_TYPE, channel: str, text: str):
    """Create a real custom-emoji message and forward it to the registered channel.

    The staged message is deleted from the user's private chat after forwarding.
    MTProto drop_author=True removes the original sender/forward header in the channel.
    """
    if telethon_client is None:
        raise RuntimeError('MTProto is not configured. Set API_ID and API_HASH.')
    user_id = update.effective_user.id
    # First create the exact custom-emoji entities using the existing robust resolver.
    staged = await send_real_custom_emoji_message(user_id, text)
    try:
        ...
        try:
            await context.bot.delete_message(chat_id=user_id, message_id=staged.id)
        except Exception:
            pass
    ...
    try:
        ...
        if context.user_data.get('waiting_for_post'):
            ...
            if not channels:
                ...
                return
            try:
                await send_post_via_private_forward(update, context, channels[0], update.message.text or '')
        ...
        if context.user_data.get('waiting_for_emoji'):
            if not has_active_subscription(user.id):
                ...
                return
            try:
                await send_real_custom_emoji_message(user.id, update.message.text or "")
            except Exception as exc:
                logging.exception("Custom emoji send failed")
                msg = str(exc)
                if "PREMIUM" in msg.upper() or "Premium" in msg:
                    await update.message.reply_text(
                        "❌ این ایموجی برای اکانت فعلی قابل ارسال نیست؛ اکانت تلگرام متصل به API باید Premium باشد.",
                        parse_mode="HTML"
                    )
                elif "did not preserve custom emoji entities" in msg:
                    await update.message.reply_text(
                        "❌ تلگرام Entity ایموجی پرمیوم را بعد از ارسال نگه نداشت. لاگ دقیق ثبت شد.",
                        parse_mode="HTML"
                    )
                else:

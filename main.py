...
async def send_custom_emoji_post(context: ContextTypes.DEFAULT_TYPE, channel: str | int, text: str):
    """Send [custom_emoji_id] tokens as real Telegram custom-emoji entities.

    Important: do NOT use RichMessage/Markdown here.  The Bot API's normal
    sendMessage endpoint explicitly supports MessageEntity(type=custom_emoji),
    which is the most direct and reliable representation for this bot.
    """
    ...
    # Resolve every custom emoji ID through Telegram first.  This also gives us
    # the exact fallback Unicode character Telegram expects for that ID.
    ...
    try:
        ...
        raise RuntimeError(
            f"CUSTOM_EMOJI_LOOKUP_FAILED: Telegram نتوانست Custom Emoji ها را بررسی کند: {exc}"
        ) from exc
    ...
    # Build the actual visible text and MessageEntity objects. Telegram's
    # offsets/lengths are UTF-16 code units, not Python Unicode code points.
    output_parts = []
    entities = []
    utf16_offset = 0
    source_end = 0
    ...
    for m in matches:
        before = text[source_end:m.start()]
        ...
        if sticker is None:
            raise ValueError(
                f"CUSTOM_EMOJI_NOT_FOUND: Telegram این Custom Emoji را برای ID {emoji_id} برنگرداند. "
                "ممکن است ID اشتباه/منقضی یا برای این Bot قابل دسترسی نباشد."
            )

        alt = getattr(sticker, 'emoji', None)
        if not alt:
            raise ValueError(f"CUSTOM_EMOJI_NO_ALT: برای {emoji_id} کاراکتر جایگزین وجود ندارد")

        output_parts.append(alt)
        alt_utf16_len = len(alt.encode('utf-16-le')) // 2
        entities.append(MessageEntity(
            type='custom_emoji',
            offset=utf16_offset,
            length=alt_utf16_len,
            custom_emoji_id=emoji_id,
        ))
        utf16_offset += alt_utf16_len
        source_end = m.end()

    tail = text[source_end:]
    output_parts.append(tail)
    clean_text = ''.join(output_parts)

    # One and only one real send.  No RichMessage and no fallback plain-text
    # send: a fallback would hide the actual Custom Emoji failure and make the
    # user see an ordinary emoji.
    try:
        ...
        raise RuntimeError(
            f"CUSTOM_EMOJI_SEND_FAILED: Telegram پیام را با Custom Emoji قبول نکرد: {exc}"
        ) from exc

    # Telegram accepted the entity. Log the exact IDs used for Render debugging.
    logging.info(
        "Custom emoji post sent successfully: chat_id=%s custom_emoji_ids=%s message_id=%s",
        chat_id, ids, getattr(message, 'message_id', None)
    )
    return message

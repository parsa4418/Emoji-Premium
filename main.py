...
async def send_real_custom_emoji_message(user_id, text):
    """Send [emoji_id] placeholders as real Telegram Custom Emoji entities.

    Telethon/Telegram entity offsets are UTF-16 based. Build the final text
    first and calculate every entity offset against that final text so Persian,
    emoji and multiple custom emojis do not produce EntityBoundsInvalidError.
    """
    ...
    for match in pattern.finditer(text):
        ...
        docs = await telethon_client(GetCustomEmojiDocumentsRequest([eid]))
        if not docs:
        ...
        # Entity offsets used by Telegram are UTF-16 code-unit offsets.
        current_text = "".join(parts)
        offset_utf16 = len(current_text.encode("utf-16-le")) // 2
        length_utf16 = len(alt.encode("utf-16-le")) // 2

        parts.append(alt)
        entities_data.append((offset_utf16, length_utf16, eid))
        last = match.end()
    ...
    entities = [
        ...
        for offset, length, eid in entities_data
    ]

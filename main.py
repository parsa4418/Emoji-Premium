...
async def send_custom_emoji_post(context: ContextTypes.DEFAULT_TYPE, channel: str, text: str):
    ...
    for m in matches:
        ...
        # NOTE: the bracket text must be exactly the fallback emoji itself
        # (e.g. "![😎](tg://emoji?id=...)"), NOT a description like
        # "Image: 😎" — Telegram rejects/ignores malformed alt text, which
        # made every sendRichMessage call fail before falling back.
        pieces.append(f'![{alt}](tg://emoji?id={emoji_id})')

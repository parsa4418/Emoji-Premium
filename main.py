...
def replace_emoji_ids(text):
    """Convert [CUSTOM_EMOJI_ID] markers to Telegram Bot API HTML tags.

    The visible fallback is intentionally kept as a normal emoji. Telegram
    uses the emoji-id to render the real custom emoji when the bot is allowed
...
async def replace_emoji_ids_with_real_fallbacks(text):
    """Same conversion, but asks Telegram for each custom emoji's real
    fallback character first. This prevents every unknown/unavailable custom
    emoji from degrading to the same 😎 character.
    """
    if not text:
        return text

    ids = list(dict.fromkeys(re.findall(r"\[(\d+)\]", text)))
    if not ids or telethon_client is None:
        return replace_emoji_ids(text)

    try:
        documents = await telethon_client(GetCustomEmojiDocumentsRequest(
            document_id=[int(x) for x in ids]
        ))
        fallbacks = {}
        for doc in documents:
            for attr in getattr(doc, "attributes", []) or []:
                ...
                if alt:
                    fallbacks[str(doc.id)] = alt
                    break

        def repl(match):
            emoji_id = match.group(1)
            fallback = fallbacks.get(emoji_id, "😎")
            return f'<tg-emoji emoji-id="{emoji_id}">{fallback}</tg-emoji>'

        return re.sub(r"\[(\d+)\]", repl, text)
    except Exception:
        logging.exception("Could not resolve custom emoji fallbacks")
        return replace_emoji_ids(text)
...
async def send_post_via_private_forward(update: Update, context: ContextTypes.DEFAULT_TYPE, channel: str, text: str):
    """
    Stage the exact Bot API message in the user's private chat, then forward
    that exact Telegram message through MTProto with drop_author=True.

    This is a real Telegram forward, not copyMessage.
    """
    if telethon_client is None:
        raise RuntimeError("MTProto client is not connected. Check API_ID/API_HASH and startup logs.")

    user_id = update.effective_user.id

    # 1) Create the source message using Bot API so Telegram creates the
    # custom-emoji entities exactly as the bot normally sends them.
    staged = await context.bot.send_message(
        chat_id=user_id,
        text=text,
        parse_mode="HTML",
    ...
    try:
        # 2) Resolve both peers through the SAME MTProto bot session.
        # This avoids relying on an entity cache left over from startup.
        source_peer = await telethon_client.get_input_entity(user_id)
        target_peer = await telethon_client.get_input_entity(channel)

        # 3) Confirm that the message exists from MTProto's point of view.
        source_message = await telethon_client.get_messages(
            source_peer, ids=staged.message_id
        )
        if not source_message:
            raise RuntimeError(
                f"MTProto could not read staged message {staged.message_id} from {user_id}."
            )

        # 4) REAL forward. drop_author removes the original author header.
        ...
        logging.info(
            "REAL FORWARD OK: source=%s message_id=%s target=%s drop_author=True",
            user_id, staged.message_id, channel
        )
        return forwarded

    except Exception:
        logging.exception(
            "REAL FORWARD FAILED: source=%s message_id=%s target=%s",
            user_id, staged.message_id, channel
        )
        raise
    finally:
        # The staging message is temporary; deleting it does not affect the
        # already-forwarded message in the destination channel.
        try:
            await context.bot.delete_message(
                chat_id=user_id, message_id=staged.message_id
            )
        except Exception:
            logging.exception("Could not delete temporary staging message %s", staged.message_id)
...
async def telethon_start(app: Application):
    ...
    if not API_ID or not API_HASH:
        raise RuntimeError(
            "API_ID and API_HASH are required. Add both to Render Environment Variables."
        )
    ...
    logging.info(
        "MTProto CONNECTED: bot_id=%s username=@%s",
        me.id, me.username or "unknown"
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if context.user_data.get('waiting_for_post'):
            ...
            if channel:
                try:
                    final_text = await replace_emoji_ids_with_real_fallbacks(update.message.text)
...
def main():
    ...
    logging.info("Starting Telegram bot in WEBHOOK mode on port %s", port)
    if webhook_url:
        logging.info("Telegram webhook URL: %s", webhook_url)
    else:
        logging.warning("RENDER_EXTERNAL_URL is missing; webhook_url will be auto-derived by Telegram endpoint configuration.")

    # PTB's built-in webhook server is used directly. No polling/getUpdates.
    app.run_webhook(
        listen="0.0.0.0",
        port=port,

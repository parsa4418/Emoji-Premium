...
async def show_increase_balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    balance_text = (
        ...
        f'حداقل مبلغ افزایش موجودی 20,000 تومان هست[5834643712189141114]\n'
        ...
        if query.data == "check_join":
            ...
            if action == "delete":
                ...
                try:
                    ...
                    if amount_int < 20000:
                        await query.answer("⚠️ حداقل مبلغ 20,000 تومان است!", show_alert=True)
                        return
                    else:
        ...
        if context.user_data.get('waiting_for_post'):
            ...
            if channel:
                try:
                    ...
                    # تلگرام اجازه نمیده بات مستقیم ایموجی پرمیوم رو توی کانال بفرسته.
                    # به همین خاطر اول پیام رو توی همین PV (که مجازه) میفرستیم،
                    # بعد همون پیام رو با forward_message به کانال منتقل میکنیم
                    # تا ایموجی پرمیومش سالم بمونه.

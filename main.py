...
# WEBHOOK_URL can be set explicitly in Environment Variables if preferred.
WEBHOOK_URL = os.getenv("WEBHOOK_URL") or os.getenv("RENDER_EXTERNAL_URL") or "https://emoji-premium.onrender.com"
WEBHOOK_PATH = "/telegram/webhook"
WEBHOOK_SECRET = os.getenv("WEBHOOK_SECRET")

# The PTB Application and its asyncio loop live in a dedicated thread.
...
def telegram_webhook():
    ...
    try:
        ...
        asyncio.run_coroutine_threadsafe(bot_application.process_update(update), bot_loop)
        return "OK", 200
    except Exception:
        logging.getLogger(__name__).exception("Webhook update processing failed")
        return "Bad Request", 400

def run_web():
    app.run(host="0.0.0.0", port=int(os.environ.get("PORT", 10000)), threaded=True, use_reloader=False)
...
async def send_post_via_private_forward(update: Update, context: ContextTypes.DEFAULT_TYPE, channel: str, text: str):
    """
    Send the post to the user's private chat first, then perform a REAL MTProto
    forward to the selected channel with drop_author=True.

    Bot API's forwardMessage has no "Hide Sender Name" option. Telegram's
    MTProto messages.forwardMessages does expose the drop_author flag, which
    removes the original author from the forwarded message while keeping it a
    real forward. This is important for preserving the original message
    entities, including custom emoji entities.
    """
    ...
    staged_message = await context.bot.send_message(
    ...
    try:
        await telethon_client.forward_messages(
            entity=channel,
            messages=staged_message.message_id,
        ...
        try:
            await context.bot.delete_message(
                chat_id=update.effective_user.id,
                message_id=staged_message.message_id,
            )
        except Exception:
            pass
...
async def start_bot(application):
    ...
    # Application.post_init is only invoked by run_polling/run_webhook.
    # We use a custom Flask webhook, so initialize MTProto explicitly.
    await telethon_start(application)
    ...
    logging.getLogger(__name__).warning("Bot started in WEBHOOK mode: %s", webhook_url)


def bot_thread_main(application):
    global bot_loop
    bot_loop = asyncio.new_event_loop()
    asyncio.set_event_loop(bot_loop)
    try:
        bot_loop.run_until_complete(start_bot(application))
        bot_loop.run_forever()
    finally:
        try:
            bot_loop.run_until_complete(application.stop())
            bot_loop.run_until_complete(telethon_stop(application))
...
def main():
    ...
    Thread(target=bot_thread_main, args=(application,), daemon=True, name="telegram-bot").start()
    run_web()

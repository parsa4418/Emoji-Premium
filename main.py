...
async def send_post_via_private_copy(update: Update, context: ContextTypes.DEFAULT_TYPE, channel: str, text: str):
    """
    Stage the post in the user's private chat with the bot, then copy it to the channel.
    Telegram's copyMessage is used instead of forwardMessage so the original sender
    is not shown in the channel.
    """
        ...
        if context.user_data.get('waiting_for_post'):
            ...
            if channel:
                try:
                    ...
                    await send_post_via_private_copy(

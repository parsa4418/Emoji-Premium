...
async def send_custom_emoji_post(context: ContextTypes.DEFAULT_TYPE, channel: str | int, text: str):
    """Send a custom-emoji post after validating the real Telegram destination."""
    ...
    if not matches:
        try:
            ...
            raise RuntimeError(f"SEND_FAILED: ارسال پیام ساده به {chat_id} شکست خورد: {exc}") from exc
    ...
    try:
        ...
        raise RuntimeError(f"CUSTOM_EMOJI_LOOKUP_FAILED: دریافت ایموجی‌های پرمیوم شکست خورد: {exc}") from exc
    ...
    for m in matches:
        ...
        if sticker is None:
            raise ValueError(f"CUSTOM_EMOJI_NOT_FOUND: ایموجی پرمیوم پیدا نشد: {emoji_id}")
        alt = getattr(sticker, 'emoji', None)
        if not alt:
            raise ValueError(f"CUSTOM_EMOJI_NO_ALT: ایموجی {emoji_id} مقدار جایگزین ندارد")
    ...
    # Build Telegram's officially supported custom_emoji entities.
    entities_payload = []
    pos_utf16 = 0
    last_match_end = 0
    for m in matches:
        ...
        emoji_id = m.group(1)
        alt = getattr(sticker_by_id[emoji_id], 'emoji', None)
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        # Primary: Bot API Rich Messages. The current Bot API supports sendRichMessage.
        rich_markdown_parts = []
        last = 0
        for m in matches:
            rich_markdown_parts.append(text[last:m.start()])
            emoji_id = m.group(1)
            alt = getattr(sticker_by_id[emoji_id], 'emoji', None)
            # Telegram's official rich-markdown syntax accepts an image-style
            # custom emoji node with tg://emoji?id=...
            rich_markdown_parts.append(f'![Image: {alt}](tg://emoji?id={emoji_id})')
            last = m.end()
        rich_markdown_parts.append(text[last:])
        rich_markdown = ''.join(rich_markdown_parts)

        rich_url = f'https://api.telegram.org/bot{token}/sendRichMessage'
        rich_payload = {
            'chat_id': chat_id,
            'rich_message': json.dumps({'markdown': rich_markdown, 'is_rtl': True}, ensure_ascii=False),
        }
        ...
        if rich_response.status_code == 200 and rich_data.get('ok'):
            logging.info('Custom-emoji RichMessage sent to %s', chat_id)
            return rich_data.get('result')

        # Fallback: classic sendMessage + MessageEntity custom_emoji.
    ...
    if send_response.status_code == 200 and send_data.get('ok'):
        logging.info('Custom-emoji sendMessage/entities sent to %s', chat_id)
    ...
    if 'not a member' in str(rich_error).lower() or 'not a member' in str(classic_error).lower():
        raise RuntimeError(
            f"BOT_NOT_MEMBER: Telegram هنوز این بات را عضو مقصد نمی‌داند. "
            f"chat_id={chat_id}; RichMessage={rich_error}; sendMessage={classic_error}"
        )
    raise RuntimeError(
        f'Telegram could not send custom emoji post. chat_id={chat_id}; '

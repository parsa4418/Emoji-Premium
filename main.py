...
async def send_real_custom_emoji_message(user_id, text):
    """Send message with real Custom Emoji entities instead of HTML fallback."""
    ...
    for eid in ids:
        placeholder = f"[{eid}]"
        pos = clean.find(placeholder)
        if pos == -1:
            continue
        docs = await telethon_client(GetCustomEmojiDocumentsRequest([int(eid)]))
        alt = "😎"
        if docs and getattr(docs[0], "attributes", None):
            for attr in docs[0].attributes:
                if hasattr(attr, "alt"):
                    alt = attr.alt
                    break
        clean = clean[:pos] + alt + clean[pos+len(placeholder):]
        entities.append(MessageEntityCustomEmoji(offset=pos, length=len(alt), document_id=int(eid)))

    await telethon_client.send_message(user_id, clean, formatting_entities=entities)

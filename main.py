...
def replace_emoji_ids(text):
    """
    Legacy helper used by the bot's UI messages. It intentionally keeps the
    existing Bot API HTML behavior for menus/help/payment text. The post
    sending path DOES NOT use this function; it builds real
    MessageEntityCustomEmoji entities through MTProto.
    """
    if not text:
        return text
    pattern = r'\[(\d+)\]'
...
def _utf16_len(value: str) -> int:
    """Telegram message entity offsets/lengths are measured in UTF-16 units."""
    return len(value.encode("utf-16-le")) // 2


async def build_mtproto_custom_emoji_message(text: str):
    """
    Convert [CUSTOM_EMOJI_DOCUMENT_ID] placeholders into real Telegram
    MessageEntityCustomEmoji objects. No visual fallback such as 😎 is used.
    ...
    documents = await telethon_client(GetCustomEmojiDocumentsRequest(document_id=ids))
    ...
    for document_id in ids:
        ...
        if document is None:
            raise ValueError(
                f"Custom emoji document not found: {document_id}. "
                "The ID may be invalid or unavailable to this bot."
            )
        alt = None
        ...
        if not alt:
            raise ValueError(
                f"Custom emoji {document_id} has no valid alt emoji. "
                "Telegram requires a real emoji character inside the entity."
            )
        alts[document_id] = alt
    ...
    for match in matches:
        before = text[source_pos:match.start()]
        parts.append(before)
        plain_offset += _utf16_len(before)

        document_id = int(match.group(1))
        alt = alts[document_id]
        entity_offset = plain_offset
        parts.append(alt)
        entity_length = _utf16_len(alt)
        entities.append(
    ...
    logging.getLogger(__name__).info(
        "CUSTOM EMOJI BUILD: ids=%s entities=%s text=%r",
        ids, len(entities), plain_text,
    )
...
async def send_post_via_private_forward(update: Update, context: ContextTypes.DEFAULT_TYPE, channel: str, text: str):
    """
    Stage the post in the user's PV using Telethon with REAL
    MessageEntityCustomEmoji entities, then REAL-forward that exact Telegram
    message to the selected channel using MTProto with drop_author=True.

    The staging message is deleted after forwarding. There is deliberately no
    [ID] -> 😎 fallback in this path: if an emoji ID cannot be resolved, the
    operation fails instead of silently changing the emoji.
    """
    if telethon_client is None:
        raise RuntimeError(
            "MTProto forward is not configured. Set API_ID and API_HASH in Render Environment Variables."
        )

    user_id = update.effective_user.id
    logger = logging.getLogger(__name__)

    plain_text, entities = await build_mtproto_custom_emoji_message(text)

    # IMPORTANT: send the staging message through MTProto itself so the
    # custom emoji entities are explicit on the actual Telegram message.
    staged_message = await telethon_client.send_message(
        entity=user_id,
        message=plain_text,
    ...
    try:
        # Re-read the exact Telegram message. This lets us verify that the
        # custom emoji entities survived Telegram's message creation.
        source_message = await telethon_client.get_messages(
            user_id, ids=staged_message.id
        )
        if not source_message:
            raise RuntimeError("Staged Telegram message could not be read back")

        source_entities = getattr(source_message, "entities", None) or []
        custom_entities = [
        ...
        logger.info(
            "STAGED MESSAGE VERIFIED: user=%s message_id=%s entities=%r custom_emoji_entities=%r",
            user_id,
            source_message.id,
            source_entities,
            custom_entities,
        ...
        expected_custom_count = len(entities)
        if len(custom_entities) != expected_custom_count:
            raise RuntimeError(
                "Telegram did not preserve all custom emoji entities in the staging message: "
                f"expected={expected_custom_count}, actual={len(custom_entities)}"
            )

        # REAL MTProto forward. This is NOT copyMessage/copy_message.
        # drop_author=True hides the original author while keeping the
        # forwarded message as a real Telegram forward.
        forwarded = await telethon_client.forward_messages(
            entity=channel,
        ...
        forwarded_items = forwarded if isinstance(forwarded, list) else [forwarded]
        logger.info(
            "REAL FORWARD OK: source=%s:%s target=%s forwarded_ids=%s drop_author=True",
            user_id,
            source_message.id,
            channel,
            [getattr(item, "id", None) for item in forwarded_items],
        )
        return forwarded
    finally:
        try:
            await telethon_client.delete_messages(user_id, [staged_message.id])
        except Exception:
            logger.exception(
                "Could not delete staging message user=%s message_id=%s",
                user_id, staged_message.id
            )


async def telethon_start(app: Application):
    global telethon_client
    if not API_ID or not API_HASH:
        raise RuntimeError(
            "API_ID and API_HASH are required for real Forward + Hide Sender. "
    ...
    logging.getLogger(__name__).info("MTProto forward client started")


async def telethon_stop(app: Application):
    global telethon_client
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if context.user_data.get('waiting_for_post'):
            ...
            if channel:
                try:
                    # IMPORTANT: do not run replace_emoji_ids() here. That helper
                    # converts IDs into HTML/fallback text and would destroy the
                    # exact Custom Emoji entity we need to forward.
                    await send_post_via_private_forward(
                        update, context, channel, update.message.text
                    )
...
def main():
    ...
    application = (
        Application.builder()
        .token(BOT_TOKEN)
        .post_init(telethon_start)
        .post_shutdown(telethon_stop)
        .build()
    )
    application.add_handler(CommandHandler("start", start))
    application.add_handler(CommandHandler("help", show_help))
    application.add_handler(CommandHandler("agency", show_agency))
    application.add_handler(CommandHandler("support", show_support))
    application.add_handler(CallbackQueryHandler(button_callback))
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_message))
    application.add_handler(MessageHandler(filters.PHOTO, handle_message))

    # Render Web Service: listen on the assigned PORT and receive Telegram
    # updates through a real webhook. No getUpdates/polling is used.
    port = int(os.environ.get("PORT", "10000"))
    webhook_path = os.environ.get("WEBHOOK_PATH", "telegram/webhook").strip("/")
    explicit_webhook_url = os.environ.get("WEBHOOK_URL", "").rstrip("/")
    render_url = os.environ.get("RENDER_EXTERNAL_URL", "").rstrip("/")
    webhook_url = explicit_webhook_url or (
        f"{render_url}/{webhook_path}" if render_url else None
    )
    secret_token = os.environ.get("WEBHOOK_SECRET", "").strip() or None

    if not webhook_url:
        raise RuntimeError(
            "Webhook URL is missing. Render should provide RENDER_EXTERNAL_URL, "
            "or set WEBHOOK_URL manually."
        )

    logging.getLogger(__name__).info(
        "Starting Render webhook: listen=0.0.0.0 port=%s url=%s path=%s",
        port, webhook_url, webhook_path,
    )

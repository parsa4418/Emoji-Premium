        ...
        for row in (ready_rows.data or []):
            if not row.get("notified", False):
                ...
                try:
                    ...
                    # اگر کاربر قبلاً /start نزده یا بات را بلاک کرده باشد،
                    # تلگرام 403 می‌دهد. این سفارش را دوباره هر ۱۰ ثانیه امتحان نکن؛
                    # خود سفارش و امکان برداشت آن باید در دیتابیس دست‌نخورده بماند.
                    error_text = str(e)
                    if "bot can't initiate conversation with a user" in error_text or "bot was blocked by the user" in error_text:
                        try:
                            supabase.table("jewelry_orders").update({"notified": True}).eq("id", row["id"]).execute()
                            logging.warning(
                                f"اعلان PV انگشتر {row.get('id')} ارسال نشد چون کاربر PV را شروع نکرده/بات را بلاک کرده؛ "
                                "از تلاش تکراری جلوگیری شد."
                            )
                        except Exception as db_error:
                            logging.error(f"خطا در علامت‌گذاری سفارش انگشتر {row.get('id')}: {db_error}")
                    else:

...
def _gift_free():
    try:
        rows=supabase.table("premium_gifts").select("*").is_("owner_id","null").limit(1000).execute().data or [];return random.choice(rows) if rows else None
...
def _gift_id():
    for _ in range(50):
        ...
        try:
            if not supabase.table("premium_gifts").select("instance_id").eq("gift_id",n).limit(1).execute().data:return n
        except Exception:pass
...
def _gift_buy(uid):
    ...
    gid=_gift_id();code=int(row["gift_code"]);value=_gift_value_from_id(gid,code)
    try:
        if update_diamonds(uid,-GIFT_BOX_PRICE) is None:return False,"❌ کسر مبلغ انجام نشد."
        up=supabase.table("premium_gifts").update({"owner_id":int(uid),"gift_id":gid,"current_value":value,"base_value":value,"acquired_price":GIFT_BOX_PRICE,"active":False,"acquired_at":datetime.now(TEHRAN_TZ).isoformat()}).eq("instance_id",row["instance_id"]).is_("owner_id","null").execute()
...
def set_active_gift(uid,iid):
    ...
    except Exception:return False,"خطا در فعال‌سازی گیفت."
...
def sell_gift(uid,iid):
    ...
    except Exception:return False,"فروش گیفت ناموفق بود."
...
def transfer_gift(sender,target,iid):
    ...
    except Exception:return False,"انتقال گیفت ناموفق بود."
...
def _gift_name(gift_code):
    try:
        idx = PREMIUM_GIFT_CODES.index(int(gift_code)) + 1
        if idx <= 50:
            return f"Snoop Dog {idx}"
        return f"Snake Box {idx - 50}"
    except (ValueError, TypeError):
        return "Premium Gift"

def _gift_profit_text(g):
    current = int(g.get("current_value") or 0)
    acquired = int(g.get("acquired_price") or GIFT_BOX_PRICE)
    diff = current - acquired
    if diff > 0:
        return f"📈 سود: +{diff:,} 💎"
    if diff < 0:
        return f"📉 ضرر: {diff:,} 💎"
...
def gifts_text(uid):
    ...
    return "🎁 گیفت‌های من\n\n" + "\n".join(
        f"{_gift_emoji(g['gift_code'],'✨')} {_gift_name(g['gift_code'])}"
        for g in gs[:50]
    )
...
def text_gift_box(message):
    ...
    m=types.InlineKeyboardMarkup();m.add(colored_button("خرید گیفت باکس 🎁",callback_data=f"giftbuy|{uid}"));show_or_edit_panel(message,uid,"giftbox",gift_box_text(uid),m,parse_mode="HTML")
...
def gift_mine_callback(call):
    ...
    for g in gs[:50]:
        label=f"{_gift_emoji(g['gift_code'],'✨')} {_gift_name(g['gift_code'])}"
        m.row(colored_button(label,callback_data=f"giftshow|{uid}|{g['instance_id']}"))
    m.row(colored_button("🏠 منوی اصلی",callback_data=f"mainmenu|{uid}"));bot.answer_callback_query(call.id);safe_edit_message(gifts_text(uid),call.message.chat.id,call.message.message_id,reply_markup=m,parse_mode="HTML")
...
def gift_show_callback(call):
    ...
    g=gs[0]
    current=int(g.get("current_value") or 0); acquired=int(g.get("acquired_price") or GIFT_BOX_PRICE)
    m=types.InlineKeyboardMarkup()
    m.row(colored_button("✨ نمایش گیفت کنار نام",callback_data=f"giftactive|{uid}|{iid}"))
    m.row(colored_button("💰 فروش گیفت",callback_data=f"giftsell|{uid}|{iid}"))
    m.row(colored_button("🔄 انتقال گیفت",callback_data=f"gifttransfer|{uid}|{iid}"))
    m.row(colored_button("🔙 گیفت‌های من",callback_data=f"giftmine|{uid}"))
    text=(f"{_gift_emoji(g['gift_code'],'✨')} {_gift_name(g['gift_code'])}\n\n"
          f"💎 قیمت فعلی: {current:,} 💎\n"
          f"🧾 قیمت خرید: {acquired:,} 💎\n"
          f"{_gift_profit_text(g)}\n"
          f"🆔 آیدی نسخه: {int(g['gift_id']):,}\n"
          f"📌 وضعیت: {'فعال کنار نام' if g.get('active') else 'غیرفعال'}")
    safe_edit_message(text,call.message.chat.id,call.message.message_id,reply_markup=m,parse_mode="HTML")
...
def gift_sell_callback(call):
    ...
    gs=[g for g in get_user_gifts(uid) if int(g["instance_id"])==iid]
    if not gs:
        bot.answer_callback_query(call.id,"این گیفت دیگر در مالکیت شما نیست.",show_alert=True);return
    g=gs[0]
    m=types.InlineKeyboardMarkup()
    m.row(colored_button("✅ بله، مطمئنم",callback_data=f"giftsellconfirm|{uid}|{iid}"))
    m.row(colored_button("❌ لغو",callback_data=f"giftshow|{uid}|{iid}"))
    safe_edit_message(f"⚠️ از فروش این گیفت مطمئنید؟\n\n{_gift_emoji(g['gift_code'],'✨')} {_gift_name(g['gift_code'])}\n💰 مبلغ دریافتی: {int(g.get('current_value') or 0):,} 💎",call.message.chat.id,call.message.message_id,reply_markup=m,parse_mode="HTML")

@bot.callback_query_handler(func=lambda c:c.data.startswith("giftsellconfirm|"))
def gift_sell_confirm_callback(call):
    uid=check_panel_owner(call)
    if uid is None:return
    iid=int(call.data.split("|")[2])
    ok,r=sell_gift(uid,iid)
    if not ok:
        bot.answer_callback_query(call.id,r,show_alert=True);return
    bot.answer_callback_query(call.id,"گیفت فروخته شد ✅")
    safe_edit_message(gifts_text(uid),call.message.chat.id,call.message.message_id,reply_markup=None,parse_mode="HTML")

import { createD1Compat } from "./db.js";
...
async function appFetch(request, env, ctx) {
  try {
    ...
    if (request.method === "POST" && url.pathname === "/webhook") {
      ...
      const task = handleUpdate(update, env, ctx).catch((err) => {
...
const env = {
  ...
  DB: createD1Compat(pool),
};

if (!env.BOT_TOKEN || !env.ADMIN_ID || !env.ADMIN_SECRET) {
  console.error("BOT_TOKEN, ADMIN_ID and ADMIN_SECRET are required.");
  process.exit(1);
}
...
server.listen(port, "0.0.0.0", () => {
  console.log(`Kade Bot running on port ${port}`);
});

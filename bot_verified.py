...
# تشخیص هویت واقعی باتی که این سرویس با آن به Telegram وصل شده است.
# این لاگ توکن را نمایش نمی‌دهد؛ فقط username و numeric id را ثبت می‌کند.
def _log_bot_identity():
    try:
        me = bot.get_me()
        logging.info("ACTIVE TELEGRAM BOT: @%s | id=%s | name=%s", me.username, me.id, me.first_name)
    except Exception:
        logging.exception("Could not verify active Telegram bot identity")
...
def _gift_buy(uid):
    ...
    try:
        ...
        up=supabase.table("premium_gifts").update({"owner_id":int(uid),"gift_id":gid,"current_value":value,"base_value":value,"acquired_price":value,"active":False,"acquired_at":datetime.now(TEHRAN_TZ).isoformat()}).eq("instance_id",row["instance_id"]).is_("owner_id","null").execute()
...
def gifts_text(uid):
    ...
    return "🎁 گیفت‌های شما\n\n"+"\n".join(f"{i}. {_gift_emoji(g['gift_code'],'✨')} | آیدی: {int(g['gift_id']):,} | ارزش: {int(g['current_value']):,} 💎"+(" ✅ فعال" if g.get('active') else "") for i,g in enumerate(gs,1))
...
def gift_mine_callback(call):
    ...
    for g in gs[:50]:m.row(colored_button(f"✨ {int(g['current_value']):,}",callback_data=f"giftshow|{uid}|{g['instance_id']}"))
...
def gift_show_callback(call):
    ...
    g=gs[0];m=types.InlineKeyboardMarkup();m.row(colored_button("👤 نمایش کنار اسم",callback_data=f"giftactive|{uid}|{iid}"));m.row(colored_button("💰 فروش",callback_data=f"giftsell|{uid}|{iid}"),colored_button("🔄 انتقال",callback_data=f"gifttransfer|{uid}|{iid}"));m.row(colored_button("🔙 گیفت‌های من",callback_data=f"giftmine|{uid}"));safe_edit_message(f"{_gift_emoji(g['gift_code'],'✨')} گیفت شما\n\n┘─ آیدی گیفت : {int(g['gift_id']):,}\n┘─ ارزش فعلی : {int(g['current_value']):,} 💎\n┘─ وضعیت : {'فعال کنار اسم' if g.get('active') else 'غیرفعال'}",call.message.chat.id,call.message.message_id,reply_markup=m,parse_mode="HTML")
...
if __name__ == "__main__":
    # آدرس وبهوک را از Render بگیر؛ آدرس ثابت قدیمی باعث می‌شد تلگرام
    # آپدیت‌ها را به سرویس دیگری بفرستد و این بات هیچ پیامی دریافت نکند.
    # در صورت نیاز می‌توان WEBHOOK_URL را به‌صورت Environment Variable نیز تنظیم کرد.
    render_url = os.environ.get("RENDER_EXTERNAL_URL", "").strip().rstrip("/")
    configured_url = os.environ.get("WEBHOOK_URL", "").strip().rstrip("/")
    WEBHOOK_URL = configured_url or (f"{render_url}/webhook" if render_url else "")

    if not WEBHOOK_URL:
        raise RuntimeError(
            "WEBHOOK_URL یا RENDER_EXTERNAL_URL تنظیم نشده است؛ "
            "برای اجرای Webhook روی Render باید یکی از این متغیرها وجود داشته باشد."
        )
    ...
    try:
        ...
        logging.info("Telegram webhook set successfully: %s", WEBHOOK_URL)
    except Exception as e:

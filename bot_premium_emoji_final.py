...
# ================== Premium Emoji Gifts ==================
# منبع اصلی گیفت‌ها: جدول premium_emoji_codes
# هر ردیف جدول یک مدل/ایموجی پرمیوم است و emoji_id شناسه واقعی Custom Emoji تلگرام است.
GIFT_BOX_PRICE = 100_000_000
GIFT_DISPLAY_STOCK = 1_000
GIFT_TABLE = "premium_emoji_codes"


def _premium_emoji_tag(emoji_id: str | int, fallback="🎁"):
    """HTML tag for Telegram Custom Emoji."""
    return f'<tg-emoji emoji-id="{html.escape(str(emoji_id))}">{fallback}</tg-emoji>'


def _premium_emoji_rows(owner_id=None):
    try:
        q = supabase.table(GIFT_TABLE).select("id,gift_name,emoji_number,emoji_id,owner_id")
        if owner_id is None:
            q = q.is_("owner_id", "null")
        else:
            q = q.eq("owner_id", int(owner_id))
        return q.order("gift_name").order("emoji_number").execute().data or []
    except Exception as exc:
        logging.error("premium emoji query failed: %s", exc)
        return []


def _premium_emoji_find(emoji_row_id, owner_id=None):
    try:
        q = supabase.table(GIFT_TABLE).select("*").eq("id", int(emoji_row_id)).limit(1)
        if owner_id is not None:
            q = q.eq("owner_id", int(owner_id))
        result = q.execute().data or []
        return result[0] if result else None
    except Exception as exc:
        logging.error("premium emoji find failed: %s", exc)
        return None
...
def gift_box_text(uid):
    return (
        ...
        f"📦 موجودی کل: {GIFT_DISPLAY_STOCK:,} عدد\n\n"
...
def _gift_buy(uid):
    ...
    # فقط از رکوردهایی انتخاب می‌کنیم که هنوز مالک ندارند.
    available = _premium_emoji_rows(owner_id=None)
    if not available:
        return False, "❌ فعلاً هیچ گیفتی باقی نمانده است.", None

    row = random.choice(available)
    try:
        if update_diamonds(uid, -GIFT_BOX_PRICE) is None:
            return False, "❌ کسر مبلغ انجام نشد.", None

        updated = (
            supabase.table(GIFT_TABLE)
            .update({"owner_id": int(uid)})
            .eq("id", int(row["id"]))
            .is_("owner_id", "null")
            .execute()
        )
        if not updated.data:
            ...
            return False, "❌ این گیفت همزمان توسط کاربر دیگری گرفته شد.", None
        return True, "✅ گیفت با موفقیت خریداری شد.", updated.data[0]
    except Exception as exc:
        update_diamonds(uid, GIFT_BOX_PRICE)
        logging.error("premium gift buy failed: %s", exc)
        return False, "❌ خرید گیفت ناموفق بود.", None


def get_user_gifts(uid):
    return _premium_emoji_rows(owner_id=uid)


def get_active_gift(uid):
    # در جدول فعلی ستون active نداریم؛ اولین گیفت کاربر به عنوان گیفت نمایشی استفاده می‌شود.
...
def display_gift_for_user(uid):
    ...
    return _premium_emoji_tag(gift["emoji_id"], "✨") if gift else ""


def gifts_text(uid):
    gifts = get_user_gifts(uid)
    if not gifts:
        return "🎁 هنوز هیچ گیفت پرمیومی نداری."

    lines = ["🎁 گیفت‌های شما", ""]
    for gift in gifts:
        name = str(gift.get("gift_name") or "گیفت")
        number = int(gift.get("emoji_number") or 0)
        emoji_id = str(gift.get("emoji_id") or "")
        lines.append(f"{_premium_emoji_tag(emoji_id, '🎁')} | {html.escape(name)} {number}")
    return "\n".join(lines)


def set_active_gift(uid, instance_id):
    gift = _premium_emoji_find(instance_id, owner_id=uid)
    if not gift:
...
def sell_gift(uid, instance_id):
    ...
    if not gift:
        return False, "❌ این گیفت متعلق به شما نیست."
    try:
        updated = supabase.table(GIFT_TABLE).update({"owner_id": None}).eq("id", int(instance_id)).eq("owner_id", int(uid)).execute()
        if not updated.data:
            return False, "❌ فروش گیفت انجام نشد."
        update_diamonds(uid, GIFT_BOX_PRICE)
        return True, GIFT_BOX_PRICE
    except Exception as exc:
        logging.error("premium gift sell failed: %s", exc)
...
def transfer_gift(sender_id, target_id, instance_id):
    ...
    gift = _premium_emoji_find(instance_id, owner_id=sender_id)
    if not gift:
        return False, "❌ این گیفت متعلق به شما نیست."
    try:
        updated = (
            supabase.table(GIFT_TABLE)
            .update({"owner_id": int(target_id)})
            .eq("id", int(instance_id))
            .eq("owner_id", int(sender_id))
            .execute()
        )
        if not updated.data:
            return False, "❌ انتقال گیفت انجام نشد."
        return True, updated.data[0]
    except Exception as exc:
        logging.error("premium gift transfer failed: %s", exc)
...
def _premium_gift_main_markup(uid):
    markup = types.InlineKeyboardMarkup()
    markup.row(
        colored_button("🎁 خرید گیفت", callback_data=f"giftbuy|{uid}"),
...
@bot.callback_query_handler(func=lambda call: call.data == "giftbox" or call.data.startswith("giftbox|"))
def premium_gift_box_callback(call):
    ...
    safe_edit_message(
        gift_box_text(uid), call.message.chat.id, call.message.message_id,
        reply_markup=_premium_gift_main_markup(uid), parse_mode="HTML"
    )
...
def text_premium_gift(message):
    ...
    if not get_user(uid):
        ...
        return
    show_or_edit_panel(
        message, uid, "premium_gifts", gift_box_text(uid),
        _premium_gift_main_markup(uid), parse_mode="HTML"
    )
...
def premium_gift_buy_callback(call):
    ...
    if uid is None:
        return
    ok, result_message, gift = _gift_buy(uid)
    if not ok:
        bot.answer_callback_query(call.id, result_message, show_alert=True)
        return

    bot.answer_callback_query(call.id, "✅ گیفت پرمیوم خریداری شد.")
    prefix = "🎁 گیفت جدید شما: "
    emoji_char = "✨"
    text = prefix + emoji_char + "\n\n" + f"🏷 {gift['gift_name']} {int(gift['emoji_number'])}"
    entity = types.MessageEntity(
        type="custom_emoji",
        offset=len(prefix),
        length=1,
        custom_emoji_id=str(gift["emoji_id"]),
    )
    try:
        bot.send_message(uid, text, entities=[entity])
    except Exception as exc:
        logging.error("custom emoji send failed: %s", exc)
        safe_send_message(uid, _premium_emoji_tag(gift["emoji_id"], "✨") + f" {html.escape(gift['gift_name'])} {int(gift['emoji_number'])}", parse_mode="HTML")

    safe_edit_message(
        "✅ گیفت پرمیوم دریافت شد.\n\n" + gift_box_text(uid),
...
def premium_gift_mine_callback(call):
    ...
    for gift in gifts[:50]:
        label = f"{gift.get('gift_name', 'گیفت')} {int(gift.get('emoji_number') or 0)}"
        markup.row(colored_button(label, callback_data=f"giftshow|{uid}|{gift['id']}"))
    markup.row(colored_button("🎁 بخش گیفت", callback_data=f"giftbox|{uid}"))
    markup.row(colored_button("🏠 منوی اصلی", callback_data=f"mainmenu|{uid}"))
    bot.answer_callback_query(call.id)
    safe_edit_message(gifts_text(uid), call.message.chat.id, call.message.message_id, reply_markup=markup, parse_mode="HTML")
...
def premium_gift_show_callback(call):
    ...
    try:
        ...
        return
    gift = _premium_emoji_find(row_id, owner_id=uid)
    if not gift:
        bot.answer_callback_query(call.id, "❌ گیفت پیدا نشد.", show_alert=True)
        return
    markup = types.InlineKeyboardMarkup()
    markup.row(colored_button("📩 ارسال ایموجی پرمیوم", callback_data=f"giftactive|{uid}|{row_id}"))
    markup.row(
        colored_button("💰 فروش", callback_data=f"giftsell|{uid}|{row_id}"),
        colored_button("🔄 انتقال", callback_data=f"gifttransfer|{uid}|{row_id}"),
    )
    markup.row(colored_button("🔙 گیفت‌های من", callback_data=f"giftmine|{uid}"))
    text = (
        f"{_premium_emoji_tag(gift['emoji_id'], '🎁')} گیفت شما\n\n"
        f"🏷 {html.escape(str(gift['gift_name']))} {int(gift['emoji_number'])}\n"
        f"🆔 Emoji ID: <code>{html.escape(str(gift['emoji_id']))}</code>"
    )
...
def premium_gift_activate_callback(call):
    ...
    if not gift:
        bot.answer_callback_query(call.id, "❌ گیفت پیدا نشد.", show_alert=True)
        return
    prefix = "🎁 ایموجی پرمیوم شما: "
    text = prefix + "✨"
    entity = types.MessageEntity(type="custom_emoji", offset=len(prefix), length=1, custom_emoji_id=str(gift["emoji_id"]))
    try:
        bot.send_message(uid, text, entities=[entity])
        bot.answer_callback_query(call.id, "✅ ایموجی پرمیوم ارسال شد.")
    except Exception as exc:
        logging.error("custom emoji send failed: %s", exc)
        bot.answer_callback_query(call.id, "❌ ارسال ایموجی پرمیوم انجام نشد.", show_alert=True)
...
def premium_gift_sell_callback(call):
    ...
    try:
        ...
        bot.answer_callback_query(call.id, "❌ گیفت نامعتبر.", show_alert=True)
        return
    ok, result = sell_gift(uid, row_id)
    if not ok:
        bot.answer_callback_query(call.id, result, show_alert=True)
        return
    bot.answer_callback_query(call.id, f"✅ {int(result):,} 💎 دریافت شد.")
    safe_edit_message(gifts_text(uid), call.message.chat.id, call.message.message_id, reply_markup=_premium_gift_main_markup(uid), parse_mode="HTML")
...
def premium_gift_transfer_callback(call):
    ...
    if not gift:
        bot.answer_callback_query(call.id, "❌ گیفت پیدا نشد.", show_alert=True)
        return
    bot.answer_callback_query(call.id, "آیدی عددی کاربر مقصد را ارسال کن.")
    register_timed_next_step_handler(
        call.message,
        _premium_gift_transfer_input,
        uid,
        row_id,
...
def _premium_gift_transfer_input(message, sender_id, row_id):
    try:
        target_id = int((message.text or "").strip())
    except Exception:
        bot.reply_to(message, "❌ آیدی عددی نامعتبر است.")
        return
    ok, result = transfer_gift(sender_id, target_id, row_id)
    bot.reply_to(message, "✅ گیفت منتقل شد." if ok else result)
...
def telegram_webhook():
    try:
        # اگر secret تنظیم شده باشد، فقط درخواست دارای هدر صحیح پذیرفته می‌شود.
        ...
        logging.exception("Telegram webhook update processing failed")
        # پاسخ 200 مانع retry بی‌نهایت تلگرام برای خطاهای داخلی می‌شود.
        return "OK", 200
